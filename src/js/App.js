...
class App extends React.Component {
  ...
  handleSubmit(filters) {
    console.log(filters)
    let filteredDataJSON = this.state.dataJSON.filter(x => {
      return Object.keys(filters).every(criterion => {
        return x.filters[criterion] && filters[criterion].some(filter => x.filters[criterion].includes(filter))
      })
    });
    this.setState({
      filteredDataJSON: filteredDataJSON
    });
    this.onChange(filteredDataJSON);

    if(Object.keys(filters).length > 0) {
      window.scrollBy({ 
        top: 430, // could be negative value
        left: 0, 
        behavior: 'smooth' 
      });
    }
  ...
  renderLaptop() {
    if (this.state.dataJSON === undefined) {
      ...
      return (
        <div className="app">
          <Cover dataURL="https://cdn.protograph.pykih.com/7613998bc8dd2fc20f94/cover.json" onSubmit={x => this.handleSubmit(x)} />
